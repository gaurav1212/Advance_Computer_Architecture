...
void X86CpuReadConfig(void)
{
	...
	cpu_decode_width = (int*) xmalloc(sizeof(int)*x86_cpu_num_cores);
	cpu_issue_width  = (int*) xmalloc(sizeof(int)*x86_cpu_num_cores);
	cpu_dispatch_width  = (int*) xmalloc(sizeof(int)*x86_cpu_num_cores);
	cpu_commit_width  = (int*) xmalloc(sizeof(int)*x86_cpu_num_cores);
	...
	for (int i=0; i< x86_cpu_num_cores;i++)
	{
		...
		//fprintf(stderr,strcat("DecodeWidth","Cpu0"));
		strcat(field,core_str);
		cpu_decode_width[i]=config_read_int(config,section,field,4);
	 	
		strcpy(field,"IssueWidth");
		strcat(field,core_str);
		cpu_issue_width[i]=config_read_int(config,section,field,4);
		
		strcpy(field,"DispatchWidth");
		strcat(field,core_str);
		cpu_dispatch_width[i]=config_read_int(config,section,field,4);
	
		strcpy(field,"CommitWidth");
		strcat(field,core_str);
		cpu_commit_width[i]=config_read_int(config,section,field,4);
	...
	fetch_queue_size = (int*) xmalloc(sizeof(int)*x86_cpu_num_cores);
	uop_queue_size = (int*) xmalloc(sizeof(int)*x86_cpu_num_cores);
    rob_size =	(int*) xmalloc(sizeof(int)*x86_cpu_num_cores);
    iq_size =	(int*) xmalloc(sizeof(int)*x86_cpu_num_cores);
    lsq_size =	(int*) xmalloc(sizeof(int)*x86_cpu_num_cores);
	...
	for (int i=0; i< x86_cpu_num_cores;i++)
	{
		...
		//fprintf(stderr,strcat("DecodeWidth","Cpu0"));
		strcat(field,core_str);
		fetch_queue_size[i]=config_read_int(config,section,field,64);
        
	 	strcpy(field,"UopQueueSize");
		strcat(field,core_str);
		uop_queue_size[i]=config_read_int(config,section,field,32);
	 	
		strcpy(field,"RobSize");
		strcat(field,core_str);
		rob_size[i]=config_read_int(config,section,field,64);
     
		strcpy(field,"IqSize");
		strcat(field,core_str);
		iq_size[i]=config_read_int(config,section,field,40);
     
		strcpy(field,"LsqSize");
		strcat(field,core_str);
		lsq_size[i]=config_read_int(config,section,field,20);
...
//GAURAV CHANGED HERE
#define DUMP_THREAD_STRUCT_STATS_PERCORE(NAME, ITEM) { \
	fprintf(f, #NAME ".Size = %d\n", (int) ITEM##_size[thread->core->id]); \
	if (x86_cpu_occupancy_stats) \
		fprintf(f, #NAME ".Occupancy = %.2f\n", asTiming(self)->cycle ? \
...
static void X86DumpCpuConfig(FILE *f)
{
	...
	for (int i=0; i< x86_cpu_num_cores;i++)
	{
		...
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,cpu_decode_width[i]);
     
		strcpy(field,"IssueWidth");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,cpu_issue_width[i]);
       
		strcpy(field,"DispatchWidth");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,cpu_dispatch_width[i]);
     
		strcpy(field,"CommitWidth");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,cpu_commit_width[i]);
	...
	for (int i=0; i< x86_cpu_num_cores;i++)
	{
		...
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,fetch_queue_size[i]);

		strcpy(field,"RobSize");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,rob_size[i]);
     
		strcpy(field,"IqSize");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,iq_size[i]);
     
		strcpy(field,"LsqSize");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,lsq_size[i]);

     	strcpy(field,"RfIntSize");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,reg_file_int_size[i]);

    	strcpy(field,"RfFPSize");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,reg_file_fp_size[i]);
	...
	for (int i=0; i< x86_cpu_num_cores;i++)
	{
		...
		strcat(field,core_str);
		strcat(field, " =%s\n");
		fprintf(f,field,trace_cache_present[i]?"True":"False");
	
	    strcpy(field,"Sets");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,trace_cache_num_sets[i]);
		    
		strcpy(field,"Assoc");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,trace_cache_assoc[i]);

	    strcpy(field,"TraceSize");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,trace_cache_trace_size[i]);

	    strcpy(field,"BranchMax");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,trace_cache_branch_max[i]);

	    strcpy(field,"QueueSize");
		strcat(field,core_str);
		strcat(field, " =%d\n");
		fprintf(f,field,trace_cache_queue_size[i]);
...
void X86CpuDumpReport(X86Cpu *self, FILE *f)
{
	...
	for (i = 0; i < x86_cpu_num_cores; i++)
	{
	    ...
	    //		"Dispatch", x86_cpu_dispatch_width);
	    		"Dispatch", cpu_dispatch_width[core->id]);

		/* Issue stage */
		/* GAURAV CHANGED HERE */
		fprintf(f, "; Issue stage\n");
		X86CpuDumpUopReport(self, f, core->num_issued_uinst_array,
	     		"Issue", cpu_issue_width[core->id]);
		//		"Issue", x86_cpu_issue_width[0]);
		...
		//		"Commit", x86_cpu_commit_width);
				"Commit", cpu_commit_width[core->id]);
...
//GAURAV CHANGED HERE
#define UPDATE_THREAD_OCCUPANCY_STATS_PERCORE(ITEM) { \
	thread->ITEM##_occupancy += thread->ITEM##_count; \
	if (thread->ITEM##_count == ITEM##_size[thread->core->id]) \
		thread->ITEM##_full++; \
}
...
void X86CpuUpdateOccupancyStats(X86Cpu *self)
{
	...
	for (i = 0; i < x86_cpu_num_cores; i++)
	{
		...
		if (x86_rob_kind == x86_rob_kind_shared)
			...
			UPDATE_CORE_OCCUPANCY_STATS_PERCORE(rob);
		if (x86_iq_kind == x86_iq_kind_shared)
			//UPDATE_CORE_OCCUPANCY_STATS(iq);
			UPDATE_CORE_OCCUPANCY_STATS_PERCORE(iq);
		if (x86_lsq_kind == x86_lsq_kind_shared)
			//UPDATE_CORE_OCCUPANCY_STATS(lsq);
			UPDATE_CORE_OCCUPANCY_STATS_PERCORE(lsq);
		if (x86_reg_file_kind == x86_reg_file_kind_shared)
		{
			//UPDATE_CORE_OCCUPANCY_STATS(reg_file_int);
			UPDATE_CORE_OCCUPANCY_STATS_PERCORE(reg_file_int);
			//UPDATE_CORE_OCCUPANCY_STATS(reg_file_fp);
			UPDATE_CORE_OCCUPANCY_STATS_PERCORE(reg_file_fp);
		}
		...
		{   
			...
			thread = core->threads[j];
			if (x86_rob_kind == x86_rob_kind_private)
				UPDATE_THREAD_OCCUPANCY_STATS_PERCORE(rob);
				//UPDATE_THREAD_OCCUPANCY_STATS(rob);
			if (x86_iq_kind == x86_iq_kind_private)
				UPDATE_THREAD_OCCUPANCY_STATS_PERCORE(iq);
				//UPDATE_THREAD_OCCUPANCY_STATS(iq);
			if (x86_lsq_kind == x86_lsq_kind_private)
				UPDATE_THREAD_OCCUPANCY_STATS_PERCORE(lsq);
				//UPDATE_THREAD_OCCUPANCY_STATS(lsq);
			if (x86_reg_file_kind == x86_reg_file_kind_private)
			{
				//UPDATE_THREAD_OCCUPANCY_STATS(reg_file_int);
				UPDATE_THREAD_OCCUPANCY_STATS_PERCORE(reg_file_int);
				//UPDATE_THREAD_OCCUPANCY_STATS(reg_file_fp);
				UPDATE_THREAD_OCCUPANCY_STATS_PERCORE(reg_file_fp);
			}
		}
	}
}
