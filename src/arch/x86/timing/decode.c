...
static void X86ThreadDecode(X86Thread *self)
{
	...
	int i;
    /* GAURAV CHANGED HERE */
	//fprintf(stderr,"CPU with id %d has a decode width %d\n",core->id,cpu_decode_width[core->id]);
	//for (i = 0; i<x86_cpu_decode_width ; i++)
	for (i = 0; i<cpu_decode_width[core->id]; i++)
	{
		/* Empty fetch queue, full uop_queue */
		if (!list_count(fetchq))
			break;
		//GAURAV CHANGED HRE
		if (list_count(uopq) >= uop_queue_size[core->id])// x86_uop_queue_size)
			break;
